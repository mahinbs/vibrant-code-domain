...
const ReviewsPage = () => {
  ...
  const filteredReviews = selectedService === "All" ? reviews : reviews.filter(review => review.service === selectedService);
  ...
  return <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black relative overflow-hidden">
      ...
      <section className="pt-20 pb-12 px-6 relative">
        <div className="container mx-auto text-center relative z-10">
          ...
          }].map((stat, index) => <div key={index} className="group relative transform hover:scale-105 transition-all duration-500" style={{
      ...
      <section className="px-6 mb-12 relative z-10">
        <div className="container mx-auto">
          <div className="flex flex-wrap justify-center gap-4 mb-8 relative">
            ...
            {services.map(service => <Button key={service} onClick={() => setSelectedService(service)} className={`relative overflow-hidden transition-all duration-500 transform hover:scale-105 ${selectedService === service ? "bg-gradient-to-r from-cyan-500 to-blue-600 text-white shadow-lg shadow-cyan-500/30 border-cyan-400/50" : "bg-black/30 backdrop-blur-xl border-cyan-500/30 text-gray-300 hover:bg-cyan-500/10 hover:border-cyan-400/50 hover:text-cyan-300"} rounded-full px-6 py-3 font-medium border`}>
                {selectedService === service && <div className="absolute inset-0 bg-gradient-to-r from-cyan-500/20 to-blue-600/20 animate-pulse"></div>}
                <span className="relative z-10">{service}</span>
                {/* Neon glow effect */}
                {selectedService === service && <div className="absolute inset-0 bg-cyan-400/20 blur-xl rounded-full animate-pulse"></div>}
              </Button>)}
          </div>
        </div>
      </section>
      ...
      <section className="px-6 pb-20 relative z-10">
        <div className="container mx-auto">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredReviews.map((review, index) => <div key={review.id} className="group relative transform transition-all duration-700 hover:scale-105" style={{
            animationDelay: `${index * 0.1}s`
                ...
                <div className="relative bg-black/40 backdrop-blur-xl rounded-3xl p-8 border border-cyan-500/20 hover:border-cyan-400/60 transition-all duration-700 hover:shadow-2xl hover:shadow-cyan-400/20 transform-gpu perspective-1000 hover:rotateY-2 hover:rotateX-2">
                  ...
                  <div className="flex mb-6 space-x-1">
                    {[...Array(review.rating)].map((_, i) => <Star key={i} className="h-5 w-5 text-yellow-400 fill-current transform hover:scale-125 transition-transform duration-300 animate-pulse" style={{
                  ...
                  <div className="flex items-center relative">
                    <div className="relative mr-4">
                      <img src={review.image} alt={review.name} className="w-16 h-16 rounded-full border-2 border-cyan-400/50 shadow-lg shadow-cyan-400/20 group-hover:shadow-cyan-400/40 transition-all duration-500" />
                      {/* Avatar glow effect */}
